...
### 项目团队与角色
- **项目经理/架构师**：cyclone_dll（负责整体设计、抓取与存储模块，经验：5 年 .NET 开发，贡献 60% 代码）。
- **开发工程师**：YuanZun Zhang（负责分类、UI 和语音模块，经验：4 年 ML & WPF，贡献 40% 代码）。
- **测试/运维**：团队内部交叉测试，无专职 QA，但使用 NUnit 进行自动化测试。
- **外部贡献**：开源社区提供反馈，GitHub Issues 管理需求，总计处理 20+ Issue。
