...
internal sealed class NewsReader : IDisposable
{
    ...
    // 配置文件路径（%APPDATA%\Vorcyc\Metis\newsreader.config.json）
    private static readonly string ConfigDir =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Vorcyc", "Metis");
    private static readonly string ConfigPath = Path.Combine(ConfigDir, "newsreader.config.json");
    ...
    /// <summary>
    /// 朗读完成时的事件（将播放状态置为 false，并在 AutoPlay 打开且未被取消的情况下自动播放下一条）。
    /// </summary>
